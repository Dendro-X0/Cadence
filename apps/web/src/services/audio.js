...
export function beep(freq, ms) {
    const Ctx = window.webkitAudioContext ?? AudioContext;
    const ctx = new Ctx();
...
export function chime(kind) {
    if (kind === 'end') {
        beep(660, 100);
        window.setTimeout(() => beep(880, 130), 140);
    }
    else {
        beep(523.25, 120);
        window.setTimeout(() => beep(659.25, 120), 150);
        window.setTimeout(() => beep(783.99, 160), 300);
    }
}
