...
export function renderTemplatesGallery(host: HTMLDivElement, handlers: {
  templates: readonly SessionTemplate[]
  onStart: (tpl: SessionTemplate) => void
  onSave: (tpl: SessionTemplate) => Promise<void>
  ...
  list.innerHTML = ''
  handlers.templates.forEach((tpl: SessionTemplate) => {
    ...
    const del = row.querySelector<HTMLButtonElement>('button[data-tpl-del]')!
    const pinBtn = row.querySelector<HTMLButtonElement>('button[data-tpl-pin]')!
