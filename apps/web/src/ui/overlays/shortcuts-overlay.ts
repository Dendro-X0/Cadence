 ...
 */
export function showShortcutsOverlay(prefs: Settings): void {
  ...
  const close = wrap.querySelector<HTMLButtonElement>('#ov-close')!
  const remove = (): void => { if (wrap.parentElement) wrap.parentElement.removeChild(wrap) }
  close.onclick = () => remove()
  wrap.addEventListener('click', (e) => { if (e.target === wrap) remove() })
  const onKey = (e: KeyboardEvent): void => { if (e.key === 'Escape') { remove(); window.removeEventListener('keydown', onKey) } }
