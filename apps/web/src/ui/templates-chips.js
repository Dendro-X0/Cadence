...
export function renderQuickChips(host, args) {
    ...
    host.innerHTML = html;
    Array.from(host.querySelectorAll('.chip')).forEach((c) => { c.classList.add('pop'); window.setTimeout(() => c.classList.remove('pop'), 400); });
    Array.from(host.querySelectorAll('.chip')).forEach((btn) => {
        btn.onclick = () => {
